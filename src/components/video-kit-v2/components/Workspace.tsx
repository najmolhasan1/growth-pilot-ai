...
export default function Workspace({ upload, onUpdate, onDelete }: WorkspaceProps) {
  ...
  const handleGenerateCreatorKit = async () => {
    ...
    try {
      const response = await fetch("/api/video-kit-v2/creator-package", {
        method: "POST",
  ...
  const [ytStartSeconds, setYtStartSeconds] = useState(0);
  ...
  const jumpToSegmentTimes = (timeStr: string) => {
    ...
    if (upload.sourceKind === 'youtube' && upload.youtubeVideoId) {
      setYtStartSeconds(targetSeconds);
      setYtPlayerKey((prev) => prev + 1);
      return;
    }
    if (mediaRef.current) {
  ...
  const handleTranslateCall = async () => {
    ...
    try {
      const response = await fetch("/api/video-kit-v2/translate", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
  ...
  const activeSegments = getTranslatedSegmentsList();
  const isYoutube = upload.sourceKind === 'youtube' && Boolean(upload.youtubeVideoId);

  return (
    <div id="workspace-container" className="space-y-6 overflow-hidden rounded-3xl border border-slate-200 bg-white p-4 font-sans shadow-sm sm:p-6">
      
      {/* Workspace Header Panel */}
      <div className="flex flex-col justify-between gap-4 border-b border-slate-200 pb-5 font-sans xl:flex-row xl:items-start">
        <div className="min-w-0">
          ...
          <h1 className="mt-2 max-w-full truncate font-display text-xl font-bold text-slate-900 lg:text-2xl">{upload.fileName}</h1>
          <p className="mt-1 break-words text-xs text-slate-400 font-mono">
        ...
        <div className="flex min-w-0 flex-wrap items-center gap-2">
          ...
          <div className="flex max-w-full overflow-x-auto rounded-xl border border-slate-200/50 bg-slate-100 p-1 text-xs font-semibold">
            <button
               type="button"
               onClick={() => setViewLanguage('original')}
               className={`whitespace-nowrap rounded-lg px-3 py-1.5 transition-all cursor-pointer ${
          ...
          <div className="flex max-w-full items-center gap-1 overflow-x-auto rounded-xl border border-slate-200 bg-slate-100 p-1 text-slate-700">
            <button
              type="button"
              onClick={triggerTxtExport}
              title="Download Transcript (TXT)"
              className="flex cursor-pointer items-center gap-1 rounded-lg px-2.5 py-1.5 text-[10px] font-bold uppercase transition-all hover:bg-white"
            >
          ...
          <button
            ...
            className="cursor-pointer rounded-xl border border-slate-200 p-2 text-red-500 transition-colors hover:bg-red-50"
          >
      ...
      <div className="grid grid-cols-1 items-start gap-6 2xl:grid-cols-[360px_minmax(0,1fr)]">
        
        {/* Left Side: Native Video Player & Multi-Language Translate Module (5 cols) */}
        <div id="media-translation-block" className="min-w-0 space-y-5">
          
          <div className="group relative aspect-video overflow-hidden rounded-2xl border border-slate-900 bg-slate-950 shadow-md">
            {isYoutube ? (
              <iframe
                key={ytPlayerKey}
                width="100%"
                height="100%"
                src={`https://www.youtube.com/embed/${upload.youtubeVideoId}?enablejsapi=1&start=${ytStartSeconds}`}
          ...
          <div className="mt-4 rounded-2xl border border-slate-200 bg-slate-50/50 p-4 sm:p-5">
            ...
            <div className="mt-4 flex flex-col gap-2 sm:flex-row 2xl:flex-col">
              <select
                value={targetLangCode}
                onChange={(e) => setTargetLangCode(e.target.value)}
                disabled={isTranslating}
                className="min-w-0 flex-1 rounded-xl border border-slate-200 bg-white px-3 py-2 text-sm font-medium focus:border-blue-450 focus:outline-hidden"
              >
              ...
              <button
                ...
                disabled={isTranslating}
                className="flex shrink-0 cursor-pointer items-center justify-center gap-1.5 rounded-xl bg-blue-600 px-4 py-2 text-sm font-medium text-white shadow-xs transition-all hover:bg-blue-700 disabled:pointer-events-none disabled:opacity-80"
              >
                {isTranslating ? (
                  <>
        ...
        <div id="content-display-block" className="flex min-h-[560px] min-w-0 flex-col overflow-hidden rounded-2xl border border-slate-200 shadow-xs xl:min-h-[680px]">
          
          {/* Segment display sub-tabs selectors */}
          <div className="shrink-0 border-b border-slate-200 bg-slate-50 p-2 sm:p-3">
            <div className="grid w-full grid-cols-2 gap-2 xl:grid-cols-4">
              <button
                type="button"
                onClick={() => setActiveTab('segments')}
                className={`flex min-w-0 cursor-pointer items-center justify-center gap-1.5 rounded-xl px-3 py-2 text-center text-xs font-semibold transition-all ${
                  activeTab === 'segments'
                    ? 'bg-white text-blue-600 shadow-xs border border-slate-200/50'
                    : 'text-slate-500 hover:text-slate-800'
                }`}
